...
当前公开的 `desktop-v0.3.7-beta.1` 提供 macOS Apple Silicon unsigned DMG 与 Windows x64 unsigned NSIS，包含场景库页、双 preset 部署与四个 fixture 包；冻结 sidecar 不依赖源码树即可列出或写入夹具。`desktop-v0.3.5-beta.1` 保持为历史预发布，不被覆盖。Windows 安装器使用 WebView2 download bootstrapper、禁止降级，当前不生成 MSI；两平台均尚未进行正式签名、公证或实体设备验收。普通用户按平台下载已发布的 DMG 或 setup EXE；正式 Authenticode 发行仍待 SignPath Foundation 审核和独立签名流程。
