...
- macOS GUI 候选构建已嵌入 CLI sidecar 并产出 `.app` / `.dmg`；正式签名、公证和发布仍待凭据与发布流程完成。
- Windows GUI 已配置原生 NSIS 构建，但仍属于构建级 beta，Windows 原生产物仍待 CI 验证且当前没有正式发布的安装包；底层 Windows CLI fresh deployment 仍为 `EXPLICIT_BETA`，不构成正式支持承诺。
...
- The macOS GUI candidate embeds the CLI sidecar and produces `.app` / `.dmg`; formal signing, notarization, and publication still depend on the release credentials and workflow.
- Native Windows NSIS packaging is configured but remains build-level beta; native artifact validation is still pending in CI and no Windows installer has been formally published. Windows CLI fresh deployment remains `EXPLICIT_BETA`, not a formal support commitment.
