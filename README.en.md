...
- The unified source version is `0.3.3`; [`v0.3.3`](https://github.com/Jia-Ethan/codex-keysmith/releases/tag/v0.3.3) is published as the Latest Release and includes the same-version sealed scenario bundle. The standalone CLI requires an explicit `--scenario-root` pointing at that bundle or a source directory, while the frozen sidecar embeds the same bundle; `v0.3.2` and earlier tags and assets stay unchanged. [`desktop-v0.2.0-beta.6`](https://github.com/Jia-Ethan/codex-keysmith/releases/tag/desktop-v0.2.0-beta.6) remains the current prerelease for the Apple Silicon macOS DMG and Windows x64 NSIS installer and does not include the v0.3 scenario library.
