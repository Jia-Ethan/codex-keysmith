...
- Desktop reconnect no longer generates a second, different assistant reply for the same turn. While the upstream is silent (Astra high thinking, often 30–40s), the envelope writes SSE comments so Codex does not idle-retry; Anthropic `ping` becomes `response.in_progress`. A reconnect POST with the same request fingerprint waits for or replays the in-flight generation instead of opening a second `/messages` call. If text was already shown and the stream then dies, the terminal is `response.incomplete` rather than `response.failed`, so Codex does not treat a visible answer as a failed turn. Evidence: thread `01a090d2`, two distinct `[P]` replies around a `正在重新連線 1/5`.
