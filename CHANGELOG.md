...
- Root cause of desktop `正在重新連線 5/5` + `stream_interrupted`: LGW stays silent for ~15s during gpt-5.6-sol high-effort thinking (no SSE bytes until a thinking dump). Codex's first-token timer fires. The envelope then muted the original socket (`claim_live`) so the client retried onto a dead stream; thread `01a09694` committed 6 reasoning items then died. Fix: 1s upstream read timeout emits a reasoning summary heartbeat; reconnects no longer mute the leader; skip invalid SSE JSON. Evidence: 15s gaps before each `response_item reasoning`, then 5 coalesce replays.
