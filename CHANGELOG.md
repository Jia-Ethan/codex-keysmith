...
- Anthropic `thinking_delta` is forwarded as Codex `response.reasoning_summary_text.delta`; `signature_delta` and invalid SSE JSON are skipped instead of killing the stream. Thread `01a09686` died after a successful tool turn with `upstream stream interrupted or invalid` / `正在重新連線 5/5` because gpt-5.6-sol high-effort thinking frames were treated as fatal. IncompleteRead is now clean EOF.
